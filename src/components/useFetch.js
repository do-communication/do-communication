...
const useFetch = (collectionType) => {
      ...
      uploadTask.on(
        ...
        () => {
          getDownloadURL(uploadTask.snapshot.ref).then(async (downloadURL) => {
            await addDoc(
              collection(db, collectionType, "Messages", "Messages"),
              {
            ...
            const reciever = await GetUser(userId);
            await setDoc(
              doc(db, collectionType, "Messages", "Recent", userId),
              {
                Content: sendFile.name,
                CreatedAt: serverTimestamp(),
                RecieverId: userId,
  ...
  const sendGroup = async (
    ...
    if (sendMessage.trim() !== "") {
      await addDoc(
        collection(db, collectionType, "GroupMessages", "Messages"),
        {
          Content: sendMessage,
          CreatedAt: serverTimestamp(),
          GroupId: userId,
      ...
      uploadTask.on(
        ...
        () => {
          getDownloadURL(uploadTask.snapshot.ref).then(async (downloadURL) => {
            ...
            const reciever = await GetGroup(userId);
            await setDoc(
              doc(db, collectionType, "GroupMessages", "Recent", userId),
              {
                Content: sendFile.name,
                CreatedAt: serverTimestamp(),
                GroupId: userId,
  ...
  const deleteMessage = async (selected, selectedFile, setUpdate, update) => {
    if (selected != null) {
      await deleteDoc(
        doc(db, collectionType, "Messages", "Messages", selected.id)
      );
      setUpdate(!update);
    }
    ...
    if (selectedFile != null) {
      ...
      deleteObject(desertRef)
        .then(async () => {
          ...
          await deleteDoc(
            doc(db, collectionType, "Messages", "Messages", selectedFile.id)
          );
          setUpdate(!update);
        })
        .catch((error) => {
  ...
  const deleteGroupMessage = async (
    ...
    if (selected != null) {
      await deleteDoc(
        doc(db, collectionType, "GroupMessages", "Messages", selected.id)
      );
      setUpdate(!update);
    }
  ...
  const deleteFile = async (
    ...
    if (selectedFile != null) {
      ...
      const desertRef = ref(
        storage,
        auth.currentUser.uid + "/" + selectedFile.data.FileName
      );
